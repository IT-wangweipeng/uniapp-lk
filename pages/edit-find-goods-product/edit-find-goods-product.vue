<template>
	<view class="page-container">
      ...
      <image class="c-product__image" :src="product.img" mode="aspectFill" @tap="chooseProductImage"></image>
      <view class="c-product__content">
				<input v-if="type === 2 && canEdit" class="c-product__title" type="text" placeholder="请输入商品名称" value="product.title" v-model="product.title"/>
        <view v-else class="c-product__title">{{product.title}}</view>
      </view>
    </view>
    
    <view class="c-cell-list">
      <view class="c-cell">
        <view class="c-cell__title">商品型号</view>
        <input v-if="type === 2 && canEdit" class="c-cell__input" type="text" placeholder="请输入商品分类" :value="product.category"/>
        <view v-else class="c-cell__val">{{product.goods_category}}</view>
      </view>
      <view class="c-cell">
        <view class="c-cell__title">档口号</view>
        <input v-if="type === 2 && canEdit" class="c-cell__input" type="text" placeholder="请输入档口号" :value="product.stalls"/>
        <view v-else class="c-cell__val">{{product.stalls}}</view>
      </view>
      <view class="c-cell">
        <view class="c-cell__title">求购数量</view>
        <input v-model="product.buy_num" v-if="type === 2 && canEdit" value="product.buy_num" class="c-cell__input" type="number" placeholder="请输入求购数量"/>
    ...
    <view class="c-cell-list">
      ...
      <view class="c-cell">
        ...
        <view v-else class="c-cell__val">{{product.min_price}}</view>
      </view>
      <view v-if="!isFindGoods" class="c-cell">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			const isFindGoods = parseInt(options.isFindGoods);
			console.log('isFindGoods ======= ', isFindGoods)
			if (isFindGoods) {
				this.isFindGoods = true;
			}
			
			this.isNew = options.isNew;
			this.canEdit = parseInt(options.canEdit) === 1 ? true : false
			
			console.log('canEdit =====  ', this.canEdit);
			this.product = JSON.parse(options.product)
			
		  if (this.canEdit) {
				const {product, type} = options
		    // this.canEdit = (edit === 1)
				
				this.type = parseInt(type)
				
				this.loadStyle();
				console.log('product ==== ', this.product, this.type, typeof(this.type))
		  }
		},
    methods: {
			loadStyle() {
      ...
      chooseProductImage() {
        ...
        uni.chooseImage({
            ...
            success: (res) => {
              ...
              this.product.imageUrl = res.tempFilePaths[0];
            }
        });
      },
      // 最低价减少
